...
def main():
    ...
    x_train, x_test, y_train, y_test = load_documents_files(
        path='files',
    ...
    for img_array in x_train:
        cont = cont + 1
        print(str(cont) + ' of ' + str(len_data))

        img = cv2.cvtColor(img_array, cv2.COLOR_RGB2BGR)
        images = _augment_image(img)

        images = [resize_with_padding(el, output_image_size)[0] for el in images]
        for i, img in enumerate(images):
            output_path = os.path.join('files/out_data_imgaug/',
                                       prefix + basename + "{0}_".format(cont) + "aug_{0}.jpeg".format(i + 1))
